...
# ── ImageNet Filter (MobileNetV3Small) ───────────────────────────
@st.cache_resource
def load_imagenet_filter():
    """
    Loads MobileNetV3Small pre-trained on ImageNet once and caches it.
    Used only for image validation — not for skin-lesion classification.
    """
...
# ImageNet synset groups that indicate a non-skin (non-dermoscopy) image.
# Each tuple: (group_label_for_display, list_of_partial_class_name_substrings)
_NON_SKIN_GROUPS = [
    ("vehicle",   ["car", "truck", "bus", "bicycle", "motorcycle", "ambulance",
                   "cab", "minivan", "van", "jeep", "limousine", "taxi",
                   "trailer", "tractor", "vehicle", "plane", "aircraft",
                   "airliner", "warplane", "helicopter", "ship", "boat",
                   "submarine", "tank", "train", "locomotive", "streetcar"]),
    ("animal",    ["dog", "cat", "bird", "horse", "cow", "sheep", "pig",
                   "rabbit", "hamster", "lion", "tiger", "bear", "wolf",
                   "fox", "deer", "elephant", "zebra", "giraffe", "monkey",
                   "gorilla", "chimpanzee", "fish", "shark", "whale",
                   "dolphin", "snake", "lizard", "turtle", "frog", "spider",
                   "insect", "butterfly", "bee", "ant", "beetle", "fly"]),
    ("food",      ["pizza", "burger", "sandwich", "hotdog", "pretzel",
                   "bagel", "spaghetti", "noodle", "soup", "salad",
                   "fruit", "apple", "banana", "orange", "strawberry",
                   "broccoli", "carrot", "corn", "mushroom", "cheese",
                   "ice cream", "chocolate", "cake", "bread", "coffee",
                   "wine", "beer", "cup", "bowl"]),
    ("furniture", ["chair", "sofa", "couch", "table", "desk", "bed",
                   "wardrobe", "cabinet", "shelf", "drawer", "lamp",
                   "toilet", "bathtub", "sink"]),
    ("electronics",["laptop", "computer", "keyboard", "mouse", "monitor",
                    "television", "phone", "camera", "printer", "remote",
                    "microwave", "oven", "refrigerator", "toaster"]),
    ("text/document",["envelope", "book", "magazine", "newspaper", "binder",
                      "notebook", "menu", "sign", "banner"]),
]


def _classify_imagenet_label(label: str):
    """
    Returns the non-skin group name if the ImageNet label matches a
    non-skin category, otherwise returns None.
    """
    label_lower = label.lower()
    for group_name, keywords in _NON_SKIN_GROUPS:
...
# ── Image Validation ─────────────────────────────────────────────
def validate_dermoscopy_image(uploaded_file):
    """
    Validates whether the uploaded image looks like a dermoscopy / skin
    lesion image using two stages:

    Stage 1 — Fast heuristics (aspect ratio, resolution, brightness).
    Stage 2 — MobileNetV3Small ImageNet classification: rejects the image
               if any of the top-5 predictions belong to a clearly
               non-skin category (vehicles, animals, food, etc.).
    ...
    # 1a. Aspect ratio — dermoscopy images are roughly square
    ratio = w / h
    if ratio < 0.4 or ratio > 3.0:
        return False, (
            f"The image aspect ratio ({ratio:.1f}) is unusual for a "
            "dermoscopy image."
        )

    # 1b. Minimum resolution
    if h < 50 or w < 50:
        return False, "The image is too small (minimum 50×50 pixels required)."

    # 1c. Brightness check
    gray = 0.299 * arr[:, :, 0] + 0.587 * arr[:, :, 1] + 0.114 * arr[:, :, 2]
    mean_brightness = float(gray.mean())
    if mean_brightness < 15:
        return False, "The image is too dark. Please upload a well-lit dermoscopy image."
    if mean_brightness > 248:
        return False, "The image is overexposed. Please upload a proper dermoscopy image."

    # ── Stage 2: MobileNetV3Small ImageNet filter ─────────────────
    filter_model = load_imagenet_filter()

    # Resize to 224×224 and preprocess for MobileNetV3
    ...
    preds = filter_model.predict(img_arr, verbose=0)               # (1, 1000)
    top5 = tf.keras.applications.mobilenet_v3.decode_predictions(preds, top=5)[0]
    # top5 is a list of (synset_id, label, score) tuples

    # Check each top-5 prediction
    for _, label, score in top5:
        group = _classify_imagenet_label(label)
        if group is not None:
            return False, (
                f"The uploaded image appears to contain a **{group}** "
                f"(detected: '{label}', confidence: {score:.1%}). "
                "Please upload a dermoscopy or close-up photo of a skin lesion."
            )
...
# ── Image validation ─────────────────────────────────────────────
is_valid, reason = validate_dermoscopy_image(img_file)
if not is_valid:
    st.error(
        f"⚠️ **Invalid image:** {reason}\n\n"
        "Please upload a dermoscopy image of a skin lesion "
        "(e.g. taken with a dermatoscope or a close-up photo of a skin lesion)."
    )
    render_footer()
    st.stop()
