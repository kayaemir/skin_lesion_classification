...
def validate_dermoscopy_image(uploaded_file):
    """
    Checks whether the uploaded image looks like a dermoscopy image.
    Uses only basic rules — no extra model or library needed.
    ...
    uploaded_file.seek(0)  # Reset again so the rest of the app can read it

    arr = np.array(img, dtype=np.float32)
    h, w = arr.shape[:2]

    # 1. Aspect ratio — dermoscopy images are roughly square
    ratio = w / h
    if ratio < 0.5 or ratio > 2.5:
        return False, f"The image aspect ratio ({ratio:.1f}) is unusual for a dermoscopy image."

    # 2. Minimum resolution
    if h < 50 or w < 50:
        return False, "The image is too small (minimum 50×50 pixels required)."

    # 3. Skin-tone color check using HSV
    # Convert RGB → HSV manually (numpy only, no cv2 needed)
    ...
    # Skin tones: hue roughly 0–40° or 340–360° (red-orange range)
    skin_mask = ((hue >= 0) & (hue <= 40)) | ((hue >= 340) & (hue <= 360))
    skin_ratio = float(skin_mask.sum()) / skin_mask.size

    if skin_ratio < 0.10:
        return False, (
            f"The image does not appear to contain skin tones "
            f"(skin-tone pixel ratio: {skin_ratio:.1%}). "
            "Please upload a dermoscopy image of a skin lesion."
        )

    # 4. Brightness check — dermoscopy images are well-lit
